...
> SSE 事件定义见 `docs/前端接入-后端改造清单.md`。
>
> **设计文档已随前端仓库分发**（`docs/`，副本同步自 `../MathStoryboard/docs/`）：
> - `docs/Manimatic-前端架构设计.md` —— 决策表 / 数据模型 / 组件清单 / 设计系统
> - `docs/前端接入-后端改造清单.md` —— **SSE 契约权威出处**
> - `docs/分镜实时交付-契约与前端改造.md` —— 产物契约 + 前端改造清单
>
> 少了上面这段，AI 辅助开发时会在错误的一侧找答案 —— 这是分仓库唯一真正的代价。
...
**后端那层 HTTP 服务（FastAPI）还没接**（见 `docs/前端接入-后端改造清单.md`）。
